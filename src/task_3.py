...
class Navigation:
    ...
    def __init__(self, node_name='Navigation'):
        ...
        # initializ obstacle checking flag
    ...
    def __image_callback(self, img):
        # the following code is modified on lab3 solution, red_ball_tracking.
        bridge = CvBridge()
        frame = bridge.imgmsg_to_cv2(img,"bgr8")
        sensitivity = 0
        # extract white feature
        lower = np.array([0, 0, 255-sensitivity])
        upper = np.array([255, sensitivity, 255])
        hsv = cv2.cvtColor(frame, cv2.COLOR_BGR2HSV)
        mask = cv2.inRange(hsv, lower, upper)
        clr_result = cv2.bitwise_and(frame, frame, mask=mask)
        gray = cv2.cvtColor(clr_result, cv2.COLOR_BGR2GRAY)
        a, framebw = cv2.threshold(gray, 127, 255, cv2.THRESH_BINARY)
        contours, hierarchy = cv2.findContours(framebw, cv2.RETR_TREE, cv2.CHAIN_APPROX_SIMPLE)
        ...
        if len(contours) != 0:
            for i in range(len(contours)):
                self.object_detect = 0  # initialize obstacle flag
                area = cv2.contourArea(contours[i])
                # rospy.loginfo(area)
                if area > 200000:  # when the moving obstacle is very closed to TTbot
                    self.rgb_objects_tmp = []
                    cv2.drawContours(framebw, contours, i, (0, 255, 0), 3)
                ...
                if not(cX == 0) and not(cY == 0):
                    self.object_detect = 1  # when the obstacle is very closed to TTbot, flag changes.
                else:
                    self.object_detect = 0
        cv2.imshow("white", framebw)
        cv2.waitKey(10)
    ...
    def move_ttbot(self, speed, heading, object_detect):
        ...
        if object_detect == 0:          # if the moving obstacle is far away from the TTbot,
            cmd_vel.linear.x = speed    # following speed and heading calculated from path_follower
            cmd_vel.angular.z = heading
        elif object_detect == 1:        # if the moving obstacle is closed to the TTbot,
            cmd_vel.linear.x = -0.2     # move backwards to leave space for obstacle
        ...
        while not rospy.is_shutdown():
            ...
            if len(path.poses) != 0 and not path_complete:
                while idx < len(path.poses)-1:
                    ...
                    self.move_ttbot(speed, heading, self.object_detect)
